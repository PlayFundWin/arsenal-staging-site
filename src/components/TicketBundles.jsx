    ...
    <div className="bg-lilac py-24 sm:py-32" id="tickets">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <h2 className="text-base font-semibold leading-7 text-white">Win Big.</h2>
          <p className="mt-2 text-4xl font-bold tracking-tight text-black sm:text-5xl">Enter the House of Hendrix Prize Draw</p>
        </div>
        <p className="mx-auto mt-6 max-w-2xl text-center text-lg leading-8 text-black">Unlock greater chances to win with bigger ticket bundles!</p>
        <div className="isolate mx-auto mt-10 grid max-w-md grid-cols-1 gap-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          {tiers.map((tier) => (
            <div key={tier.id} className={classNames(tier.mostPopular ? "bg-[#6366f1] ring-2 ring-indigo-900" : "bg-[#e5e0ff] ring-1 ring-white/10", "rounded-3xl p-8 xl:p-10 ")}>
              <div className="flex items-center justify-between gap-x-4">
                <h3 id={tier.id} className={classNames(tier.mostPopular ? "text-white" : "text-black", "text-lg font-semibold leading-8")}>
                  {tier.name}
                </h3>
                {tier.mostPopular ? <p className="rounded-full bg-[white] px-2.5 py-1 text-xs font-semibold leading-5 text-black">Most popular</p> : null}
              </div>
              <p className={classNames(tier.mostPopular ? "text-white" : "text-black", "mt-4 text-sm leading-6")}>{tier.description}</p>
              <p className="mt-6 flex items-baseline gap-x-1">
                <span className={classNames(tier.mostPopular ? "text-white" : "text-black", "text-4xl font-bold tracking-tight")}>£{tier.price[frequency.value]}</span>
              </p>
              <Link
                to={tier.href}
